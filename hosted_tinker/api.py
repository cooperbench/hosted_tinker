...
from hosted_tinker.config import EngineConfig, add_model, config_to_argv
from hosted_tinker.db_models import (
    CheckpointDB,
    ModelDB,
    FutureDB,
    RequestStatus,
...
async def lifespan(app: FastAPI):
    ...
    # Build subprocess command for the background engine.
    # Use the same Python interpreter to run the engine module directly.
    cmd = [sys.executable, "-m", "hosted_tinker.engine"]
...
class Datum(BaseModel):
    ...
    def to_types(self) -> types.Datum:
        ...
        seq_len = len(inp["target_tokens"].data)

        if "weights" not in inp:
            weights = types.TensorData(data=[1.0] * seq_len)
        else:
        ...
        return types.Datum(
            loss_fn_inputs=types.LossFnInputs(
                ...
                weights=weights,
                advantages=inp["advantages"].to_types() if "advantages" in inp else types.TensorData(data=[0.0] * seq_len),
                logprobs=inp["logprobs"].to_types() if "logprobs" in inp else types.TensorData(data=[0.0] * seq_len),
            ),
