...
def _init_model(base_model, lora_rank, lora_alpha, lora_targets, gradient_checkpointing, device, remove_padding):
    """Load model, apply LoRA, wrap with FSDP. Returns (model, optimizer, tokenizer, attn_impl)."""
    attn_impl = "sdpa"
        ...
        if cmd_type == "shutdown":
            ...
            for mb_start in mb_starts:
                ...
                if remove_padding and not is_dummy:
                    ...
                    position_ids = torch.tensor([flat_pos], dtype=torch.long, device=device)

                    attn_mask = torch.zeros(total_tok, total_tok, dtype=torch.bfloat16, device=device)
                    attn_mask.fill_(torch.finfo(torch.bfloat16).min)
                    off = 0
                    for sl in seq_lens_mb:
                        causal = torch.tril(torch.zeros(sl, sl, dtype=torch.bfloat16, device=device))
                        attn_mask[off : off + sl, off : off + sl] = causal
                        off += sl
                    attn_mask = attn_mask.unsqueeze(0).unsqueeze(0)

                    if compute_grad:
                        out = model(input_ids=input_ids, attention_mask=attn_mask, position_ids=position_ids, use_cache=False)
                    else:
                        with torch.no_grad():
                            out = model(input_ids=input_ids, attention_mask=attn_mask, position_ids=position_ids, use_cache=False)
...
def worker_main(local_rank, world_size, master_port, args_dict, cmd_queue, result_queue,
    ...
    if remove_padding and rank == 0:
        logger.info("remove_padding=True: packing sequences with block-diagonal attention mask")
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Pack sequences into flat tensor instead of right-padding (requires flash_attn)",
    )
